...
import { reject, reduceReject } from './../../src/reject'
...
describe('filter', () => {
  it('filters the vegetables', () => {
    const vegetables = produce.filter(product => { return product.type === 'vegetable' })
    expect(vegetables).toEqual([{name: 'cucumber', type: 'vegetable'}, {name: 'celery', type: 'vegetable'}])
  })

  it('filters the fruit', () => {
    const fruit = produce.filter(product => { return product.type === 'fruit' })
    expect(fruit).toEqual([{name: 'banana', type: 'fruit'}, {name: 'orange', type: 'fruit'}])
  })

  it('drops the celery', () => {
    const fruit = produce.filter(product => { return product.name !== 'celery' })
  ...
  it('selects vegetables which are in stock with a price less than 10', () => {
    const cart = complicatedProduce.filter(product => { return product.type === 'vegetable' && product.quantity > 0 && product.price < 10 })
    expect(cart).toEqual([])
  })
...
describe('reject', () => {
  it('rejects numbers below 15', () => {
    ...
    const numbersAboveFifteen = reject(numbers, number => { return number < 15 })
